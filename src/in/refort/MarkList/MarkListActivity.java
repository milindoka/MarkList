...
public class MarkListActivity extends Activity {
    ...
    int in1,in2,strength=20,Max,MCQmarks=0;
    String mcqscore="00",sMax="100",tempstr,Examiner="",Exam="",Subject="";
    ...
    Boolean modified=false,end=false,OpenNow=false,CleanNow=false,NewNow=false,MCQmode=false,autoscroll=true;
...
////////////////////////////
   ...
   */
   /////////////////////////////////////////////////////////////////////
    ...
    public void WarnBeforeClean()
    {    	  
         	 AlertDialog.Builder myAlertDialog = new AlertDialog.Builder(this);
	    	 myAlertDialog.setTitle("Modified !");
	    	 myAlertDialog.setMessage("Save Marklist Before Clean ?");
	    	 myAlertDialog.setPositiveButton("Yes", new DialogInterface.OnClickListener()
	    	 {
        	 public void onClick(DialogInterface arg0, int arg1) {
	    	  // do something when the OK button is clicked
    ...
    { 	if(RollIdx<2 ||RollIdx>strength+2) {show("Out of Range"); return;}
    	String temp[],stemp,temproll;
    	if(RollIdx<2 ||RollIdx>strength+2) {showtop("Out of Range"); return;}
    	stemp=Roll.get(RollIdx);
    	temp=stemp.split(":");
        temproll=Decrement(temp[0].trim());
    	Roll.add(RollIdx,temproll+ " :    ");
		modified=true;
		((BaseAdapter) lv.getAdapter()).notifyDataSetChanged();
    ...
    { 	if(RollIdx<2 ||RollIdx>strength+2) {show("Out of Range"); return;}
    	...
    	temproll=Increment(temp[0].trim());
    	
    	//int temproll=Integer.parseInt(temp[0].replaceAll("[^0-9.]",""));
    	
    	Roll.add(RollIdx+1,temproll+ " :    ");
		modified=true;
		((BaseAdapter) lv.getAdapter()).notifyDataSetChanged();
    ...
    public void onCreate(Bundle savedInstanceState)
    {
     ...
     //  strength=lroll-froll+1;
        ...
        final TextView myTitleText = (TextView) findViewById(R.id.ttext);
		 //if (myTitleText != null)
		     myTitleText.setText("MarkList 3.0 - Untitled");
     ...
     //   lv.setOnTouchListener(l)
		...
		buttonHeader.setOnClickListener(new View.OnClickListener() {
		 ...
		 final Button buttonBlank = (Button) findViewById(R.id.Blanks);
			...
			int end = lv.getLastVisiblePosition();
			String original = (String) lv.getItemAtPosition(start + 2);
			if(!original.contains(":")) { JumpBlank(); return;}
			boolean move;
			int len = original.length();
			String temp[];
			...
			{ int end=lv.getLastVisiblePosition();
			int start = lv.getFirstVisiblePosition();
			if(start>strength-2) {start=strength-2;}
			//System.out.print(end);
			String original= (String) lv.getItemAtPosition(start + 2);
			if(!original.contains(":")) JumpBlank();
			boolean move=true;
			int len=original.length();
			String temp[];
			temp=original.split(":");
	   ...
	   final Button buttonUP1 = (Button) findViewById(R.id.UP1);
			...
			final Button buttonClean = (Button) findViewById(R.id.Clean);
		  ...
		  final Button buttonABS = (Button) findViewById(R.id.ABS);
			...
			final Button buttonExtendUP = (Button) findViewById(R.id.ExtendUP);
			buttonExtendUP.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v)
				{   
					LongPressInsertAbove(2);
				   
				
				}
				});
			
			final Button buttonExtendDN = (Button) findViewById(R.id.ExtendDN);
			buttonExtendDN.setOnClickListener(new View.OnClickListener() {
				public void onClick(View v)
				{   
					LongPressInsertBelow(strength+1);
			...
			buttonMerit.setOnClickListener(new View.OnClickListener() {
			  ...
			  final Button buttonCutABS = (Button) findViewById(R.id.CutABS);
				...
				final Button buttonCutBLS = (Button) findViewById(R.id.CutBLS);
				buttonCutBLS.setOnClickListener(new View.OnClickListener() {
					public void onClick(View v)
					{ 
					CutBLS();
					}
 ...
 //////////////////////////////////////////////////////////////Non Event Functions
    ...
    {  
	    ...
	    for(i=2;i<strength+2;i++)
	    {   stemp=Roll.get(i);
	        if(!stemp.contains(":")) continue; //invalid record
           temp=stemp.split(":");
    ...
    { int i;
      String temp2;
		...
		for(i=1;i<=TotSets;i++)
		{
		aDataRow=myReader.readLine(); /// keystring
		temp=aDataRow.split(":");
		temp2=temp[1].trim();
		Set.set(i,temp2);
...
{	   int i;
    ...
    //??   tmpStr=Integer.toString(froll);
       txtData+="First Roll  : ";txtData+=froll; txtData+='\n';
  //??     tmpStr=Integer.toString(lroll);
       txtData+="Last  Roll  : ";txtData+=lroll; txtData+='\n';
